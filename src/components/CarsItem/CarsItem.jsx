...
const CarsItem = ({ objCar }) => {
  ...
  const formatLocation = [city, country, rentalCompany];
  const secondInfoCar = [type, model, mileage, newAccessories];
    ...
    <li id={id} className={css.cardItem}>
      ...
      <div className={css.infoCar}>
        {formatLocation.map(el => {
          return <p className={css.locationInfo}>{el}</p>;
        })}
      </div>

      <div className={css.infoCarSecond}>
        {secondInfoCar.map(el => {
          return <p className={css.aboutCar}>{el}</p>;
        })}
      </div>
